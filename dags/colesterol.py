...
def _to_flat_drug(rows: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    for ev in rows:
        ...
        # Coletar todas as reações adversas
        reactions = "; ".join([
...
# DAG
@dag(
    dag_id="openfda_colesterol",
    description=(
        "openFDA drug/event (atorvastatin, sinvastatin, rosuvastatin, pitavastatin) → "
        "normaliza (STAGE) → agrega diário (BQ). Jun–Jul/2024."
    ),
    
)
def openfda_colesterol():
    ...
    def extract_transform_load() -> Dict[str, str]:
        ...
        for drug in DRUGS_GENERIC:
            ...
            while day <= TEST_END:
                ...
                while True:
                    params = {
                        "search": _search_expr_by_day(day, drug),
                        "limit": str(limit),
                        "skip": str(skip),
                    }
                ...
                print(f"[fetch] {day} ({drug}): {total_dia} registros.")
                day = date.fromordinal(day.toordinal() + 1)
        print(f"[fetch] Jun–Jul/2024: {n_calls} chamadas, {len(all_rows)} registros no total.")
        ...
        # Dropar tabela STAGE para remover propriedades antigas
